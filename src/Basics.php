...
class Basics
{
    ...
    {
        $allparts = explode(' ', $str);

        if ($wordsCountToRemain > count($allparts)) {

            $allparts = array_pad($allparts, $wordsCountToRemain, end($allparts));
            return implode(' ', $allparts);

        } else {
            $answer = array_slice($allparts, 0, $wordsCountToRemain);
